...
| **ledger** (of a list) | an enumerated list held to the tree in both directions, so an entry that stops being true fails and a subject with no entry fails too. Each carries a `LEDGER:` marker; there are fifteen ([08-testing.md](08-testing.md)) |

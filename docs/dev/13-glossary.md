...
| **the oracle** (in the core) | `LeafRelations` in `crates/valgebra-core/src/decision/oracle.rs`, the trait through which the decision procedure asks the bindings about a class or a value. `crates/valgebra-py/src/oracle.rs` holds the implementation that answers |
...
| **ledger** (of a list) | an enumerated list held to the tree in both directions, so an entry that stops being true fails and a subject with no entry fails too. Each carries a `LEDGER:` marker; there are twenty-two ([08-testing.md](08-testing.md)) |

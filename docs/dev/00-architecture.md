...
| `crates/valgebra-py/src/check/walk.rs` | `MAX_LABEL_UNFOLDS` | `4` | shape | a union's branch label following a chain of references, where a definition naming another names one forever | `tests/test_union_reports.py` |
...
| `crates/valgebra-py/src/check/walk/sequence.rs` | `SNAPSHOT_MIN_ELEMENTS` | `16` | shape | a list too narrow for a snapshot of it to pay for its own allocation | its own tests, `tests/test_walk_edges.py`, and `scripts/perf_gate.py --binding` |
| `crates/valgebra-py/src/check/walk/sequence.rs` | `SNAPSHOT_MAX_ELEMENTS` | `262_144` | shape | a snapshot large enough that walking the copy costs more cache than the reference counts it avoids, and two mebibytes of transient with it | its own tests, `tests/test_walk_edges.py`, and `scripts/perf_gate.py --binding` |
...
| `crates/valgebra-core/src/ir/intern.rs` | `SLOTS` | `1024` | shape | the table of shared handles growing into a record of every schema a process has built: it is direct-mapped, so a collision evicts | its own tests, and `scripts/perf_gate.py --decision`, which reads 65% higher with the table answering nothing |
| `crates/valgebra-core/src/ir/intern.rs` | `SUMMARY` | `4` | shape | a wide list paying a hash per entry for a table that is about to compare the entries anyway; the hash chooses a slot and the comparison decides, so reading fewer costs a collision and never an answer | its own tests, and `scripts/perf_gate.py --core`, which reads 4.5% higher when the hash reads every entry |

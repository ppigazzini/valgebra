...
def test_subtyping_is_reflexive(spec: object) -> None:
    ...
    if compiled is not None:
        assert compiled.is_subtype(spec)
...
def test_bottom_below_and_top_above(spec: object) -> None:
    ...
    if compiled is None:
        return
    assert compiled.is_subtype(anything)  # s <= top
    assert validator(nothing).is_subtype(spec)  # bottom <= s
...
def test_transitivity_on_a_decided_chain() -> None:
    ...
    assert validator(int).is_subtype(union(int, str))
    assert validator(bool).is_subtype(union(int, str))


def test_antisymmetry_implies_equivalence() -> None:
    # Mutual subtyping is equivalence: bool|int and int include each other.
    left = union(bool, int)
    assert left.is_subtype(int)
    assert validator(int).is_subtype(left)
    assert left.equivalent(int)

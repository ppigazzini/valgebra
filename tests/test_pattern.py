"""Native string-pattern refinements: `Annotated[str, Pattern(...)]`.
...
from valgebra import Pattern, ValidationError, union, validator


def test_pattern_fullmatch_anchored() -> None:
    schema = validator(Annotated[str, Pattern(r"a+")])
...
def test_pattern_rejects_non_string() -> None:
    schema = validator(Annotated[str, Pattern(r"\d+")])
...
def test_pattern_matches_re_fullmatch(pattern: str) -> None:
    schema = validator(Annotated[str, Pattern(pattern)])
...
def test_invalid_pattern_raises_at_compile_time() -> None:
    with pytest.raises(ValueError, match="invalid regular expression"):
        validator(Annotated[str, Pattern(r"(unclosed")])


def test_pattern_composes_and_reaches_json() -> None:
    oid = Annotated[str, Pattern(r"[0-9a-f]{24}")]
    ...
    either = union(oid, Annotated[str, Pattern(r"[A-Z]+")])
...
def test_pattern_validate_reports_a_pattern_violation() -> None:
    with pytest.raises(ValidationError) as info:
        validator(Annotated[str, Pattern(r"\d+")]).validate("abc")
